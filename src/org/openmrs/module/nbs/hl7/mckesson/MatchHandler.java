...
public class MatchHandler extends org.openmrs.module.sockethl7listener.MatchHandler{
			...
			Date encounterDate)
	{
		...
		Provider bestProv = getBestProvider(hl7Patient, resolvedPatient,
				encounterDate);
		
		String attribute = updateAttribute(resolvedPatient, resolvedPatient.getPatientIdentifier(),
				resolvedPatient.getPersonName(), resolvedPatient.getGender(), 
				resolvedPatient.getBirthdate(), bestProv, resolvedPatient.getPersonAddress(),
				resolvedPatient.getAttribute(ATTRIBUTE_NEXT_OF_KIN), 
				resolvedPatient.getAttribute(ATTRIBUTE_TELEPHONE));
